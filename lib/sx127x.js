...
function SX127x(options) {
  ...
  this._spreadingFactor = options.spreadingFactor || 7;  // quanto menor maior, mais rápido e menor a distância, o inverso é verdadeiro
  this._signalBandwidth = options.signalBandwidth || 125E3;
  this._codingRate = options.codingRate || (4 / 5);
  this._preambleLength = options.preambleLength || 12; // estava 8, porém lendo a documentação parecia que 12 seria o default.
...
SX127x.prototype.open = function() {
...
SX127x.prototype.close = function() {
  this._spi.closeSync();
...
SX127x.prototype.readVersion = function() {
  return this._readRegister(REG_VERSION);
};

SX127x.prototype.setFrequency = function(frequency) {
...
SX127x.prototype.setLnaBoost = function(boost) {
...
SX127x.prototype.setTxPower = function(level) {
  if (level < 2) {
    level = 2;
  } else if (level > 17) {
    level = 17;
...
SX127x.prototype.setSpreadingFactor = function(sf) {
  if (sf < 6) {
    sf = 6;
  } else if (sf > 12) {
    sf = 12;
...
SX127x.prototype.setSignalBandwidth = function(sbw) {
  var bw;
...
SX127x.prototype.setCodingRate = function(cr) {
  var denominator;
...
SX127x.prototype.setPreambleLength = function(length) {
  var lengthBuffer = Buffera.alloc(2);
...
SX127x.prototype.setSyncWord = function(sw) {
  this._syncWord = sw;

  this._writeRegister(REG_SYNC_WORD, sw);
};

SX127x.prototype.setCrc = function(crc) {
  this._crc = crc;
...
SX127x.prototype._readRegister = function(register) {
  var readMessage = {
...
SX127x.prototype._readRegisterBytes = function(register, length) {
  ...
  var readMessage = {
    ...
    receiveBuffer: Buffera.alloc(sendBuffer.length),
...
SX127x.prototype._writeRegister = function(register, value) {
  var sendBuffer;
...
SX127x.prototype._onDio0Rise = function(err, value) {
  console.log("_onDio0Rise(): err:" + err + " value: " + value);
  if (err || value === 0) {
    return;
  }
