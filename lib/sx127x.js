...
class SX127x extends events.EventEmitter {
  ...
  setFrequency (frequency) {
    ...
    var frequencyBuffer = Buffer.alloc(3);

    var frf = ( frequency * Math.pow(2, 19) ) / 32e6;

    frequencyBuffer.writeInt32BE(Math.floor(frf));
       
    this._writeRegister(REG_FR_MSB, frequencyBuffer[2]);
    this._writeRegister(REG_FR_MID, frequencyBuffer[1]);
    this._writeRegister(REG_FR_LSB, frequencyBuffer[0]);
  }
